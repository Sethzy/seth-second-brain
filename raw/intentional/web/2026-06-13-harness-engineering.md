...
collected_at: 2026-06-13T10:49:12Z
...
Original URL: https://martinfowler.com/articles/exploring-gen-ai/harness-engineering.html
Fetched URL: https://martinfowler.com/articles/exploring-gen-ai/harness-engineering.html
Awesome Harness summary: Thoughtworks' framing of harness work into context engineering, architectural constraints, and "garbage collection" against entropy.
...
[![Photo of Birgitta BÃ¶ckeler](/articles/exploring-gen-ai/bb.jpg)](https://birgitta.info)
...
* [Feedforward and Feedback](#FeedforwardAndFeedback)
* [Computational vs Inferential](#ComputationalVsInferential)
* [The steering loop](#TheSteeringLoop)
* [Timing: Keep quality left](#TimingKeepQualityLeft)
* [Regulation categories](#RegulationCategories)
  + [Maintainability harness](#MaintainabilityHarness)
  + [Architecture fitness harness](#ArchitectureFitnessHarness)
  + [Behaviour harness](#BehaviourHarness)
* [Harnessability](#Harnessability)
* [Harness templates](#HarnessTemplates)
* [The role of the human](#TheRoleOfTheHuman)
* [A starting point - and open questions](#AStartingPoint-AndOpenQuestions)

### Sidebars

* [Metaphors only go so far](#MetaphorsOnlyGoSoFar)
* [How does harness engineering relate to context engineering?](#HowDoesHarnessEngineeringRelateToContextEngineering)
* [Ambient affordances](#AmbientAffordances)
* [Ashby's Law](#AshbysLaw)

---

This article [updates an earlier memo](/articles/exploring-gen-ai/harness-engineering-memo.html) outlining my first
impressions of harness engineering.
...
It has been pointed out to me that wrapping harnesses around harnesses doesn't make sense: âHave you ever tried to put a harness on the inside of a dog?â So this somewhat stretches the metaphor, but I'm happy to accept that if it proves useful to navigate the usage of this word.
...
Figure 1:
The term âharnessâ means different things depending on the bounded context.
...
* **Inferential** - Semantic analysis, AI code review, âLLM as judgeâ. Typically run by a GPU or NPU. Slower and more expensive; results are more non-deterministic.
...
My colleague Ned Letcher uses the term âambient affordancesâ for
the properties of the agent environment that make it more harnessable:
âstructural properties of the environment itself that make it legible, navigable, and tractable to agents operating within it.â
...
As human developers we bring our skills and experience as an implicit harness to every codebase. We absorbed conventions and good practices, we have felt the cognitive pain of complexity, and we know that our name is on the commit. We also carry organisational alignment - awareness of what the team is trying to achieve, which technical debt is tolerated for business reasons, and what âgoodâ looks like in this specific context. We go in small steps and at our human pace, which creates the thinking space for that experience to get triggered and applied.

A coding agent has none of this: no social accountability, no aesthetic disgust at a 300-line function, no intuition that âwe don't do it that way here,â and no organisational memory. It doesn't know which convention is load-bearing and which is just habit, or whether the technically correct solution fits what the team is trying to do.
...
* [An OpenAI team documented what their harness looks like](https://openai.com/index/harness-engineering/): layered architecture enforced by custom linters and structural tests, and recurring âgarbage collectionâ that scans for drift and has agents suggest fixes. Their conclusion: âOur most difficult challenges now center on designing environments, feedback loops, and control systems.â
* [Stripe's write-up about their minions](https://stripe.dev/blog/minions-stripes-one-shot-end-to-end-coding-agents) describes things like pre-push hooks that run relevant linters based on a heuristic, they highlight how important âshift feedback leftâ is to them, and their âblueprintsâ show how they're integrating feedback sensors into the agent workflows.
...
* I hear stories from teams at Thoughtworks about tackling architecture drift with both computational and inferential sensors, e.g. increasing API quality with a mix of agents and custom linters, or increasing code quality with a âjanitor armyâ.
