...
canonical_url: "https://cursor.com/blog/agent-best-practices/"
vendor_blog: cursor
...
Original URL: https://cursor.com/blog/agent-best-practices/
...
Jan 9, 2026·[product](/blog/topic/product)

# Best practices for coding with agents

![](/marketing-static/_next/image?url=https%3A%2F%2Fptht05hbb1ssoooe.public.blob.vercel-storage.com%2Fassets%2Favatars%2Flee-robinson.jpeg&w=48&q=70)

Lee Robinson · 17 min read
...
Cursor's agent harness orchestrates these components for each model we support. We [tune instructions](/blog/codex-model-harness) and [tools](/blog/semsearch) specifically for every frontier model based on internal evals and external benchmarks.
...
A [study](/blog/productivity) from the University of Chicago found that experienced developers are more likely to plan before generating code. Planning forces clear thinking about what you're building and gives the agent concrete goals to work toward.
...
### [#](#starting-over-from-a-plan)Starting over from a plan
...
### [#](#let-the-agent-find-context)Let the agent find context

You don't need to manually tag every file in your prompt.

Cursor's agent has powerful [search tools](/blog/semsearch) and pulls context on demand. When you ask about "the authentication flow," the agent finds relevant files through `grep` and semantic search, even if your prompt doesn't contain those exact words.
...
### [#](#when-to-start-a-new-conversation)When to start a new conversation
...
### [#](#reference-past-work)Reference past work
...
![Reference past chats to bring in context from previous conversations](/marketing-static/_next/image?url=https%3A%2F%2Fptht05hbb1ssoooe.public.blob.vercel-storage.com%2Fassets%2Fblog%2Fpast-chats.jpg&w=3840&q=70)
...
### [#](#rules-static-context-for-your-project)Rules: Static context for your project
...
### [#](#skills-dynamic-capabilities-and-workflows)Skills: Dynamic capabilities and workflows
...
### [#](#example-long-running-agent-loop)Example: Long-running agent loop
...
### [#](#bugbot-for-pull-requests)Bugbot for pull requests
...
### [#](#native-worktree-support)Native worktree support

Cursor automatically creates and manages [git worktrees](/docs/configuration/worktrees) for parallel agents. Each agent runs in its own worktree with isolated files and changes, so agents can edit, build, and test code without stepping on each other.
...
### [#](#run-multiple-models-at-once)Run multiple models at once
...
![Multi-agent judging shows which solution Cursor recommends](/marketing-static/_next/image?url=https%3A%2F%2Fptht05hbb1ssoooe.public.blob.vercel-storage.com%2Fassets%2Fchangelog%2Fchangelog-2-2-judge.jpg&w=1920&q=70)
...
![Kanban view of Cursor Agents performing coding and research tasks.](/marketing-static/_next/image?url=https%3A%2F%2Fptht05hbb1ssoooe.public.blob.vercel-storage.com%2Fassets%2Fblog%2Fweb-and-mobile.jpg&w=1920&q=70)
...
## [#](#debug-mode-for-tricky-bugs)Debug Mode for tricky bugs
...
![Debug Mode in the agent dropdown](/marketing-static/_next/image?url=https%3A%2F%2Fptht05hbb1ssoooe.public.blob.vercel-storage.com%2Fassets%2Fchangelog%2Fchangelog-2-2-debug-dropdown.jpg&w=1920&q=70)
...
Get started with [Cursor's agent](/download) to try these techniques.
