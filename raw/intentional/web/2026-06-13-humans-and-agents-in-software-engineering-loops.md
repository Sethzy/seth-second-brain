...
collected_at: 2026-06-13T10:49:38Z
...
Original URL: https://martinfowler.com/articles/exploring-gen-ai/humans-and-agents.html
Fetched URL: https://martinfowler.com/articles/exploring-gen-ai/humans-and-agents.html
Awesome Harness summary: A clear mental model for where humans should strengthen the harness instead of micromanaging every artifact.

## Fetched Content

# Humans and Agents in Software Engineering Loops

[![Photo of Kief Morris](kief-morris.jpg)](https://kief.com)
...
Should humans stay out of the software development process and vibe code, or do we need developers in the loop inspecting every line of code? I believe the answer is to focus on the goal of turning ideas into outcomes. The right place for us humans is to build and manage the working loop rather than either leaving the agents to it or micromanaging what they produce. Letâs call this âon the loop.â

As software creators we build an outcome by turning our ideas into working software and iterating as we learn and evolve our ideas. This is the âwhy loopâ. Until the AI uprising comes humans will run this loop because weâre the ones who want what it produces.

The process of building the software is the âhow loop.â The how loop involves creating, selecting, and using intermediate artefacts like code, tests, tools, and infrastructure. It may also involve documentation like technical designs and ADRs. Weâre used to seeing many of these as deliverables, but intermediate artefacts are really just a means to an end.

![The software delivery feedback loops: An upper "why" loop connected to a lower âhowâ loop. The why loop iterates over an idea and working software. The how loop iterates over interim artefacts like specs, code, and tests.](humans-agents-image1.png)
...
![Multiple levels of âhowâ loops supporting the âwhyâ loop. An outer loop iterates on a feature. A middle loop iterates on stories. An inner loop iterates on code.](humans-agents-image2.png)
...
But maybe we donât care about the means that are used to achieve our goals? Maybe we can just let the LLMs run the how loop however they like?

## Humans outside the loop

Plenty of people have discovered the joy of letting humans stick to the why loop, and leaving the how loop for the agents to deal with. This is the common definition of âvibe codingâ. Some interpretations of Spec Driven Development (SDD) are much the same, with humans investing effort in writing the outcome we want, but not dictating how the LLM should achieve it.

![Humans outside the loop: An upper "why" loop with a human on top. The loop iterates over an idea and working software. This is connected to a lower "how" loop by a robot, which iterates over interim artefacts like code.](humans-agents-image3.png)

*Figure 3: Human runs the why loop, agent runs the how loop.*

The appeal of humans staying out of the how loop is that the why loop is the one we really care about. Software development is a messy domain that inevitably bogs down into over-engineered processes and coping with technical debt. And every new LLM model so far has gotten better at taking a user prompt and spitting out working software. If youâre not satisfied with what it spits out, tell the LLM and itâll give you another iteration.

If the LLMs can write and change code without us, do we care whether the code is âcleanâ? It doesnât matter whether a variable name clearly expresses its purpose as long as an LLM can figure it out. Maybe we donât even need to care what language the software is written in?

We care about external quality, not internal quality for its own sake. External quality is what we experience as a user or other stakeholder of the software. Functional quality is a must, the system needs to work correctly. And for production software we also care about non-functional, operational quality. Our system shouldnât crash, it should run quickly, and we donât want it posting confidential data to social media sites. We donât want to run up massive cloud hosting bills, and in many domains we need to pass compliance audits.

We care about internal quality when it affects external outcomes. When human coders were crawling through the codebase, adding features and fixing bugs, they could do it more quickly and reliably in a clean codebase. But LLMs donât care about developer experience, do they?

In theory our LLM agents can extrude a massively overcomplicated spaghetti codebase, test and fix it by running ad-hoc shell commands, and eventually produce a correct, compliant, high-performing system. We just get our swarms Ralph Wiggumming on it, running in data centers that draw energy from the boiling oceans they float on, and eventually weâll get there. [1](#fn:1)
...
![Humans in the loop: A single âwhy+howâ loop with a human at the top and a robot at the bottom. The loop iterates over idea, interim artefacts like code and tests, and the working software.](humans-agents-image4.png)

*Figure 4: Human runs the why loop and the how loop*

When people talk about âhumans in the loopâ, they often mean humans as a gatekeeper within the innermost loop where code is generated, such as manually inspecting each line of code created by an LLM.
...
We need to adopt classic âshift leftâ thinking. Once upon a time we wrote all of our code, passed it to a QA team to test, and then tried to fix enough bugs to ship a release. Then we discovered that when developers write and run tests as we work we find and fix issues right away, which makes the whole process faster and more reliable.

What works for humans can work for agents as well. Agents produce better code when they can gauge the quality of the code they produce themselves rather than relying on us to check it for them. We need to instruct them on what weâre looking for, and give them guidance on the best ways to achieve it.

## Humans on the loop

Rather than personally inspecting what the agents produce, we can make them better at producing it. The collection of specifications, quality checks, and workflow guidance that control different levels of loops inside the how loop is the agentâs harness. The emerging practice of building and maintaining these harnesses, [Harness Engineering](https://martinfowler.com/articles/exploring-gen-ai/harness-engineering.html), is how humans work on the loop.

![Humans on the loop: An upper "why" loop connected to a lower âhowâ loop by a human. The why loop iterates over an idea and working software.A robot sits at the bottom of the lower âhowâ loop, which iterates over interim artefacts like specs and code.](humans-agents-image5.png)

*Figure 5: Human defines the how loop and the agent runs it*

Something like the on the loop concept has also been described as the âmiddle loop,â including by participants of [The Future of Software Development Retreat](https://martinfowler.com/bliki/FutureOfSoftwareDevelopment.html). The middle loop refers to moving human attention to a higher-level loop than the coding loop.

The difference between in the loop and on the loop is most visible in what we do when weâre not satisfied with what the agent produces, including an intermediate artefact. The âin the loopâ way is to fix the artefact, whether by directly editing it, or by telling the agent to make the correction we want. The âon the loopâ way is to change the harness that produced the artefact so it produces the results we want.
...
![Flywheel: An upper "why" loop connected to a lower âhowâ loop by a human and a robot. The why loop iterates over an idea and working software. The how loop iterates over interim artefacts like specs.](humans-agents-image6.png)
...
At some point this might look a lot like humans out of the loop, old-school vibe coding. I suspect that will be true for standard types of work that are done often as the improvement loops reach diminishing returns. But by engineering the harness we wonât just get one-off, âgood enoughâ solutions, weâll get robust, maybe even anti-fragile systems that continuously improve themselves.

---

1. These days âralph loopâ is often used colloquially to mean just firing up a bunch of agents and leaving them to keep looping until (hopefully) they finish their task. But [as originally described](https://ghuntley.com/ralph/) the operator plays an important role in steering agents as they ralph.Â [↩](#fnref:1)
