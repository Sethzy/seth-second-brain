...
Original URL: https://www.humanlayer.dev/blog/12-factor-agents
Fetched URL: https://www.humanlayer.dev/blog/12-factor-agents
Awesome Harness summary: HumanLayer's operating principles for production agents, including explicit prompts, state ownership, and clean pause-resume behavior.

## Fetched Content

# # 12 Factor Agents

[Dex](https://twitter.com/dexhorthy) · April 3, 2025 · < 32 min read

[#agents](/blog/tags/agents)[#llm](/blog/tags/llm)[#ai](/blog/tags/ai)[#best-practices](/blog/tags/best-practices)[#architecture](/blog/tags/architecture)
...
* [How We Got Here: A Brief History of Software](#how-we-got-here-a-brief-history-of-software)
* [Factor 1: Natural Language to Tool Calls](#factor-1-natural-language-to-tool-calls)
* [Factor 2: Own Your Prompts](#factor-2-own-your-prompts)
* [Factor 3: Own Your Context Window](#factor-3-own-your-context-window)
* [Factor 4: Tools Are Just Structured Outputs](#factor-4-tools-are-just-structured-outputs)
* [Factor 5: Unify Execution State and Business State](#factor-5-unify-execution-state-and-business-state)
* [Factor 6: Launch/Pause/Resume with Simple APIs](#factor-6-launchpauseresume-with-simple-apis)
* [Factor 7: Contact Humans with Tool Calls](#factor-7-contact-humans-with-tool-calls)
* [Factor 8: Own Your Control Flow](#factor-8-own-your-control-flow)
* [Factor 9: Compact Errors into Context Window](#factor-9-compact-errors-into-context-window)
* [Factor 10: Small, Focused Agents](#factor-10-small-focused-agents)
* [Factor 11: Trigger from Anywhere](#factor-11-trigger-from-anywhere)
* [Factor 12: Make Your Agent a Stateless Reducer](#factor-12-make-your-agent-a-stateless-reducer)
...
![010-software-dag](/blog/12-factor-agents/010-software-dag.png)
...
![015-dag-orchestrators](/blog/12-factor-agents/015-dag-orchestrators.png)
...
![020-dags-with-ml](/blog/12-factor-agents/020-dags-with-ml.png)
...
![025-agent-dag](/blog/12-factor-agents/025-agent-dag.png)

And let the LLM make decisions in real time to figure out the path

![026-agent-dag-lines](/blog/12-factor-agents/026-agent-dag-lines.png)
...
![027-agent-loop-animation](/blog/12-factor-agents/027-agent-loop-animation.gif)

And the "materialized" DAG that was generated would look something like:

![027-agent-loop-dag](/blog/12-factor-agents/027-agent-loop-dag.png)
...
![028-micro-agent-dag](/blog/12-factor-agents/028-micro-agent-dag.png)
...
![029-deploybot-high-level](/blog/12-factor-agents/029-deploybot-high-level.png)
...
![033-deploybot-animation](/blog/12-factor-agents/033-deploybot.gif)
...
![035-deploybot-conversation](/blog/12-factor-agents/035-deploybot-conversation.png)
...
![040-4-components](/blog/12-factor-agents/040-4-components.png)
...
![110-natural-language-tool-calls](/blog/12-factor-agents/110-natural-language-tool-calls.png)
...
![120-own-your-prompts](/blog/12-factor-agents/120-own-your-prompts.png)
...
If the signature looks a little funny, we'll get to that in [factor 4 - tools are just structured outputs](#factor-4-tools-are-just-structured-outputs)
...
* Changes to models parameters like temperature, top\_p, frequency\_penalty, presence\_penalty, etc.
...
![140-tools-are-just-structured-outputs](/blog/12-factor-agents/140-tools-are-just-structured-outputs.png)
...
![155-unify-state](/blog/12-factor-agents/155-unify-state-animation.gif)
...
You may have things that can't go in the context window, like session ids, password contexts, etc, but your goal should be to minimize those things. By embracing [factor 3](#factor-3-own-your-context-window) you can control what actually goes into the LLM
...
![165-pause-resume](/blog/12-factor-agents/165-pause-resume-animation.gif)
...
Closely related to [factor 5 - unify execution state and business state](#factor-5-unify-execution-state-and-business-state) and [factor 8 - own your control flow](#factor-8-own-your-control-flow), but can be implemented independently.

**Note** - often AI orchestrators will allow for pause and resume, but not between the moment of tool selection and tool execution. See also [factor 7 - contact humans with tool calls](#factor-7-contact-humans-with-tool-calls) and [factor 11 - trigger from anywhere, meet users where they are](#factor-11-trigger-from-anywhere).
...
![170-contact-humans-with-tools](/blog/12-factor-agents/170-contact-humans-with-tools.png)
...
The above includes patterns from [factor 5 - unify execution state and business state](#factor-5-unify-execution-state-and-business-state), [factor 8 - own your control flow](#factor-8-own-your-control-flow), [factor 3 - own your context window](#factor-3-own-your-context-window), and [factor 4 - tools are just structured outputs](#factor-4-tools-are-just-structured-outputs), and several others.

If we were using the XML-y formatted from [factor 3 - own your context window](#factor-3-own-your-context-window), our context window after a few turns might look like this:
...
5. **Durable**: Combined with [factor 6 - launch/pause/resume with simple APIs](#factor-6-launchpauseresume-with-simple-apis), this makes for durable, reliable, and introspectable multiplayer workflows

[More on Outer Loop Agents over here](https://theouterloop.substack.com/p/openais-realtime-api-is-a-step-towards)

![175-outer-loop-agents](/blog/12-factor-agents/175-outer-loop-agents.png)

Works great with [factor 11 - trigger from anywhere, meet users where they are](#factor-11-trigger-from-anywhere)
...
![180-control-flow](/blog/12-factor-agents/180-control-flow.png)
...
* context window compaction or other [memory management](#factor-3-own-your-context-window)
...
* request\_clarification: model asked for more info, break the loop and wait for a response from a human
* fetch\_git\_tags: model asked for a list of git tags, fetch the tags, append to context window, and pass straight back to the model
* deploy\_backend: model asked to deploy a backend, this is a high-stakes thing, so break the loop and wait for human approval
...
You may notice this is closely related to [factor 5 - unify execution state and business state](#factor-5-unify-execution-state-and-business-state) and [factor 6 - launch/pause/resume with simple APIs](#factor-6-launchpauseresume-with-simple-apis), but can be implemented independently.
...
Hitting some consecutive-error-threshold might be a great place to [escalate to a human](#factor-7-contact-humans-with-tool-calls), whether by model decision or via deterministic takeover of the control flow.

![195-factor-09-errors](/blog/12-factor-agents/195-factor-09-errors.gif)
...
![1a0-small-focused-agents](/blog/12-factor-agents/1a0-small-focused-agents.png)
...
![1a5-agent-scope-grow](/blog/12-factor-agents/1a5-agent-scope-grow.gif)
...
If you're waiting for the [humanlayer](https://humanlayer.dev) pitch, you made it. If you're doing [factor 6 - launch/pause/resume with simple APIs](#factor-6-launchpauseresume-with-simple-apis) and [factor 7 - contact humans with tool calls](#factor-7-contact-humans-with-tool-calls), you're ready to incorporate this factor.

![1b0-trigger-from-anywhere](/blog/12-factor-agents/1b0-trigger-from-anywhere.png)
...
* **High Stakes Tools**: If you're able to quickly loop in a variety of humans, you can give agents access to higher stakes operations like sending external emails, updating production data and more. Maintaining clear standards gets you auditability and confidence in agents that [perform bigger better things](#what-if-llms-get-smarter)
...
![1c0-stateless-reducer](/blog/12-factor-agents/1c0-stateless-reducer.png)

![1c5-agent-foldl](/blog/12-factor-agents/1c5-agent-foldl.png)
...
How do we get AI coding agents to solve hard problems in complex, brownfield codebases?](/blog/advanced-context-engineering)
