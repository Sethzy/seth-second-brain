...
original_url: https://x.com/djfarrelly/status/2031838984813297748
author: "Dan Farrelly | Inngest.com"
handle: djfarrelly
status_id: 2031838984813297748
captured_at: 2026-06-19T21:43:37+08:00
published_at: "Wed Mar 11 21:05:38 +0000 2026"
...
# X post by @djfarrelly

## Source

- Original: [https://x.com/djfarrelly/status/2031838984813297748](https://x.com/djfarrelly/status/2031838984813297748)
- Canonical: [https://x.com/djfarrelly/status/2031838984813297748](https://x.com/djfarrelly/status/2031838984813297748)
...
The trends of sub-agents has shifted from specialized to generic, but regardless of your chosen approach, each system ([or harness if you will](https://x.com/djfarrelly/status/2028556984396452250)), needs to have three core patterns for working with sub-agents.
...
As I covered in [my X article last week](https://x.com/djfarrelly/status/2028556984396452250), the primary purpose is context compression. The Cursor team also described this in their recent [Latent Space interview](https://www.latent.space/p/cursor-2) . Sub-agents exist so the parent never has to absorb the full trajectory of a delegated task. The sub-agent might read 8 files and make 15 tool calls. The parent sees a 750-token summary.

During my own testing with the [Utah project](https://github.com/inngest/utah), sub-agents resulted in 90%+ reduction in tokens added to the parent agent's context.
...
Within [our system](https://www.inngest.com/docs/guides/invoking-functions-directly), step.invoke() is function-to-function RPC with durability. The parent checkpoints before invoking, so it won't re-invoke on retry. If the sub-agent crashes, Inngest retries it. The parent just waits.
...
- Preview: Every agentic system that actually ships ends up needing three delegation patterns: one that blocks, one that fires and forgets, and one that runs later. The question isn't whether you need sub-agents
